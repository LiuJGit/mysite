...
"""
开启Django shell，即类似于 ipython的东西
...
# 上面两条语句等价于下面要介绍的关联查询筛选语句：
PeopleInfo.objects.filter(book__id__exact=1)
...
两大特性:
1) 惰性执行;
2) 缓存: 使用同一个查询集，第一次使用时会发生数据库的查询，然后会把结果缓存下来，再次使用这个查询集时会使用缓存的数据，减少了数据库的查询次数
"""

# 第一次会查一遍BookInfo.objects.all()
[book.id for book in BookInfo.objects.all()]
# 第二次又会查一遍BookInfo.objects.all()
[book.id for book in BookInfo.objects.all()]
